...
Locations and artefacts are described purely in text.

Parts of the described location can be examined more closely for a detailed
description.

The world is populated with artefacts which can be "collected".

The locations are navigable - a graph network (not merely cardinal compass
directions) - and an edge between nodes can represent any distance.
...
Characters can be described in locations and examined more closely but you
...
Certain locations can be marked as public junctions, to/from which other nodes
can be attached. The graph is directional.

Each node has a numeric id (incremental in creation order) by which it can be
navigated to (if the user has access rights). Essentially teleporting around
Proseden.

A set of locations may be designated with a common entry-point, so that
...
Each edge leading from a node also has an incremental-in-creation-order id, as
well as a nickname. Navigation can use either the number or the name.

Artefacts are placed in locations and, like all else, are merely a text
description which can have details examined.

Each artefact is homed in a single location. It can be moved by the owner.
...
A node will have a creation date and a list of modified dates. Historical
...
A node has a single owner but others/all can be invited to read/edit/manage.

A node may appear in a single group to which those rights are assigned.

The same access rights can be assigned to a "user" level, which bestows the
right to all of that user's nodes and groups. This is a way to effectively share
all of your things with invitees/everybody.

For simplicity, groups cannot be nested. The hierarchy is user -> (optional)
group -> node.

Artefacts inherit the rights of the node they are homed in.
...
as simple as a file of destinations from each node.
